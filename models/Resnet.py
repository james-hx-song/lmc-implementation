...
class Resnet(nn.Module):
    def __init__(self, initial_features, num_blocks) -> None:
        super().__init__()
        self.in_features = initial_features
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=initial_features, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(initial_features),
        )

        self.layer1 = self._make_layer(num_blocks[0], initial_features, stride=1)
        self.layer2 = self._make_layer(num_blocks[1], initial_features*2, stride=2)
        self.layer3 = self._make_layer(num_blocks[2], initial_features*4, stride=2)
        self.fc = nn.Linear(initial_features*4, 10) # CIFAR-10

    
    def _make_layer(self, num_blocks, out_channels, stride):
        layers = []
        layers.append(Block(self.in_features, out_channels, stride))
